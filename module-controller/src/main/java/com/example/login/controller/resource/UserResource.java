...
public class UserResource {
    ...
    public ResponseEntity<UserDTO> create(@Valid @RequestBody UserForm form, UriComponentsBuilder uriBuilder) {
        UserDTO dto = mapper.toDto(interactor.create(mapper.toDomain(form)));
        return ResponseEntity.created(uriBuilder.path("/{uuid}").buildAndExpand(dto.uuid()).toUri()).body(dto);
    }
